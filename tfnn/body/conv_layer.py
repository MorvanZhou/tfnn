...
class ConvLayer(Layer):
    ...
    def construct(self, layers_configs, layers_results):
        ...
        _in_size = layers_configs['neural_structure'].iloc[-1]['output_size']
        with tfnn.variable_scope(self.name):
            ...
            with tfnn.name_scope('Wx_plus_b'):
                product = tfnn.nn.conv2d(
                    input=layers_results['final'].iloc[-1],
                    filter=self.W,
    ...
    def _check_image_shape(self, layers_configs, layers_results):
        ...
        if self.image_shape is not None:
            if len(layers_configs) == 1:
                ...
                layers_configs['neural_structure'].iloc[-1]['output_size'] = self.image_shape
                _xs_placeholder = layers_results['final'].iloc[-1]
