...
def train(data_path, duration, save_to='/tmp/'):
    ...
    network.add_hidden_layer(10, activator=tfnn.nn.relu, dropout_layer=True)
...
def traj_comparison(data_path, duration, id, model_path='tmp', model='/model'):
    df = pd.read_pickle(data_path)[['filter_position', 'Vehicle_ID', 'Frame_ID', 'deri_v', 'deri_a_clipped']].dropna()
